...
-- ============================================================================
-- These functions provide atomic point batch operations to prevent race conditions
-- during appointment booking and cancellation processes.

-- Function to atomically increment point batch remaining points (for cancellations)
...
BEGIN
  ...
  SET 
    remaining_points = point_batches.remaining_points + increment_amount,
    is_active = CASE 
      WHEN point_batches.remaining_points + increment_amount > 0 THEN true 
      ELSE point_batches.is_active 
    END,
    updated_at = NOW()
...
BEGIN
  ...
  SELECT point_batches.remaining_points INTO current_points
  FROM point_batches 
  WHERE point_batches.id = batch_id;
  ...
  UPDATE point_batches 
  SET 
    remaining_points = point_batches.remaining_points - decrement_amount,
    is_active = CASE 
      WHEN point_batches.remaining_points - decrement_amount <= 0 THEN false 
      ELSE point_batches.is_active 
    END,
    updated_at = NOW()
  WHERE point_batches.id = batch_id
...
-- Add indexes for better performance on atomic operations
...
-- Usage examples for atomic functions:
-- To deduct points during booking:
-- SELECT * FROM decrement_point_batch('batch-uuid-here', 1);
-- 
-- To refund points during cancellation:
-- SELECT * FROM increment_point_batch('batch-uuid-here', 1);
